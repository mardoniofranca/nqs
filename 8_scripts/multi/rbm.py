...
def run(i,w,rad,J,size,net,n_it, learning_rate, stoch_rec, variat_st,a):
    ...
    param_file = "data/" + str(PARAM) + "_" + str(size) + "_" + str(n_it) + "_" + str(learning_rate) + "_" + str(stoch_rec) + "_" + str(variat_st) + "_" + str(alpha) + "_" + str(net) + ".log"
    gs.run(out=param_file, n_iter=n_it)
    ...
    print(PARAM,w,j1,j2,structure_factor_gs,E_gs[0],0,np.mean(energy[-1:]))
    
    l = [PARAM,w,rad,j1,j2,structure_factor_gs,E_gs[0],0,np.mean(energy[-1:])]
